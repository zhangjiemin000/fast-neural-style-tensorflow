...
def main(_):
    ...
    with tf.Graph().as_default():
        with tf.Session().as_default() as sess:
            ...
            config_list = utils.read_conf_file(FLAGS.config_file)
            for config_file in config_list.trainning_list:
                model_file_name = config_file.split('/')[-1].split('.')[0]
                model_file_path = os.path.abspath(os.path.join('models',model_file_name,'fast-style-model.ckpt-done'))
                # Use absolute path
                # FLAGS.model_file = os.path.abspath(FLAGS.model_file)
                saver.restore(sess, model_file_path)
                generated_file = os.path.join('./generated',model_file_path.split("/")[-2]+'.jpg')
