...
def net(image, training):
    ...
    with tf.variable_scope('conv2'):
        conv2 = relu(instance_norm(conv2d(conv1, 32, 32, 3, 2)))
    with tf.variable_scope('conv3'):
        conv3 = relu(instance_norm(conv2d(conv2, 32, 32, 3, 2)))
    with tf.variable_scope('res1'):
        res1 = residual(conv3, 32, 3, 1)  #残差网络
    with tf.variable_scope('res2'):
        res2 = residual(res1, 32, 3, 1)
    with tf.variable_scope('res3'):
        res3 = residual(res2, 32, 3, 1)
    with tf.variable_scope('res4'):
        res4 = residual(res3, 32, 3, 1)
    with tf.variable_scope('res5'):
        res5 = residual(res4, 32, 3, 1)
    # print(res5.get_shape())
    with tf.variable_scope('deconv1'):
        # deconv1 = relu(instance_norm(conv2d_transpose(res5, 128, 64, 3, 2)))
        deconv1 = relu(instance_norm(resize_conv2d(res5, 32, 32, 3, 2, training)))  #重新resize大小的卷积网络
    with tf.variable_scope('deconv2'):
        # deconv2 = relu(instance_norm(conv2d_transpose(deconv1, 64, 32, 3, 2)))
        deconv2 = relu(instance_norm(resize_conv2d(deconv1, 32, 32, 3, 2, training)))
