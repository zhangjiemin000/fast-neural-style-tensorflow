...
def parse_args():
    ...
    parser.add_argument('-cc', '--cc', default=None, help='the config of step')
...
if __name__ == '__main__':
    ...
    if args.list_config is not None:  # 输入参数是否定义了list_config
        ...
        config = utils.read_conf_file(args.cc)
        for weight in range(config.min_style_weight, config.max_style_weight, config.style_weigth_step):
            FLAGS = utils.read_conf_file(args.cc)
            FLAGS.style_weight = weight
            FLAGS.naming = FLAGS.naming+"_"+str(FLAGS.style_weight)
            main(FLAGS)
